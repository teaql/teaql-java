...
Generated for `com.teaql.runtimeexampleconformanceservice`. Use this guide when adding controllers, services, jobs, or integration code in this workspace.

## Setup

```java
import com.teaql.runtimeexampleconformanceservice.Q;
import com.teaql.runtimeexampleconformanceservice.RuntimeExampleConformanceServiceUserContext;
...
```java
var list = Q.platforms()
    .selectWorkItemListWith(Q.workItems().selectSelf())
    .comment("load with workItemList")
    .purpose("fetch child workItemList")
    .executeForList(userContext);
```
...
- `Platform`: reverse relations `1`, query `Q.platforms()`, save `new Platform().auditAs("comment").save(userContext)`, request `com.teaql.runtimeexampleconformanceservice.platform.PlatformRequest`