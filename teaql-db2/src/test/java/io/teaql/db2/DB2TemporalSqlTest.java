...
public class DB2TemporalSqlTest {
    @Test public void preparedAndDiagnosticSqlAreEquivalent() throws Exception {
        String url=System.getenv("TEAQL_TEST_DB2_URL"), user=System.getenv("TEAQL_TEST_DB2_USER"), password=System.getenv("TEAQL_TEST_DB2_PASSWORD");
        Assume.assumeTrue(url != null && user != null && password != null);
        try(Connection c=DriverManager.getConnection(url,user,password)) {
            drop(c); run(c,"CREATE TABLE teaql_temporal_java_fixture(id INTEGER, d DATE, local_time TIMESTAMP(3))");
            ...
            run(c,SqlDataServiceExecutor.debugSql(sql,v,"db2").replaceFirst("VALUES \\(1,","VALUES (2,")); assertRows(c); drop(c);
        }
    }
