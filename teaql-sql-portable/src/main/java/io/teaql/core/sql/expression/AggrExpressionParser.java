...
public class AggrExpressionParser implements SQLExpressionParser<AggrExpression> {
    ...
    public String toSql(
        ...
        if (!(operator instanceof AggrFunction)) {
            throw new TeaQLRuntimeException("AggrExpression operator should be " + AggrFunction.class);
        }

        List<Expression> expressions = agg.getExpressions();
        if (CollectionUtil.size(expressions) != 1) {
            throw new TeaQLRuntimeException("AggrExpression operator should have 1 expression");
        }
        String sqlColumn =
    ...
    public String genAggrSQL(AggrFunction operator, String sqlColumn) {
        ...
        throw new TeaQLRuntimeException("unsupported agg function:" + aggrFunction);
    }
}
